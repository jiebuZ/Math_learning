...
$$
V_1 + ··· + V_m = \lbrace v_1 + ··· + v_m : v_1 \in V_1, ..., v_m \in V_m \rbrace
$$
...
### 例题2
证明： $V$ 的三个子空间的并集是 $V$ 的子空间，当且仅当其中一个包含另两个。

证明：

引理： $V$的两个子空间的并集是 $V$ 的子空间，当且仅当其中一个包含另一个。

引理的证明：

设 $V$ 的两个子空间记为 $U_1,U_2$ 。

若 $U_1\subseteqq U_2$ 或 $U_2\subseteqq U_1$ ，则 $U_1\cup U_2=U_1$ 或 $U_2$ 显然为 $V$ 的子空间。

$U_1\cup U_2$ 为 $V$ 的子空间，但 $U_1\nsubseteqq U_2$ 且 $U_2\nsubseteqq U_1$ ，则 $\exists u_1\in (U_1-U_2),u_2\in (U_2-U_1)$ ，又由 $u_1,u_2\in U_1\cup U_2$ 得 $u_1+u_2\in U_1\cup U_2$ ，于是：

若 $u_1+u_2\in U_1$ ，则 $(u_1+u_2)-u_1=u_2\in U_1$ 与 $u_2\in (U_2-U_1)$ 矛盾；

若 $u_1+u_2\in U_2$ ，则 $(u_1+u_2)-u_2=u_1\in U_2$ 与 $u_1\in (U_1-U_2)$ 矛盾；

这证明了 $U_1\subseteqq U_2$ 或 $U_2\subseteqq U_1$ 成立。

回到原题，设 $V$ 的三个子空间记为 $U_1,U_2,U_3$ 。

若其中一个子空间包含了另两个，则 $U_1\cup U_2\cup U_3=U_1$ 或 $U_2$ 或 $U_3$ ，命题显然成立。

若 $U_1\cup U_2\cup U_3$ 是 $V$ 的一个子空间，但没有任何一个子空间包含另外两个。由引理可知 $U_1\cup U_2$ 、 $U_2\cup U_3$ 、 $U_3\cup U_1$ 都不是子空间。

于是 $\exists u_1\in (U_1-(U_2\cup U_3)),u_2\in (U_2-(U_1\cup U_3))$ 。

对 $\forall k\in F$ ，考虑向量 $v_k=u_1+ku_2$ ，由 $u_1,u_2\in U$ ，则对于每一个 $k$ ， $v_k=u_1+ku_2$ 必须落在 $U_1,U_2$ 或 $U_3$ 中。

若 $v_k\in U_1$ ，则 $v_k-u_1=ku_2\in U_1$ 与 $u_1\in (U_1-(U_2\cup U_3))$ 矛盾；

若 $v_k\in U_2$ ，则 $v_k-ku_2=u_1\in U_2$ 与 $u_2\in (U_2-(U_1\cup U_3))$ 矛盾；

若 $v_k\in U_3$ ，则还存在着 $v_m=u_1+mu_2\in U_3$ ，此时 $v_k-v_m=(k-m)u_2\in U_3$ 与 $u_2\in (U_2-(U_1\cup U_3))$ 矛盾。
